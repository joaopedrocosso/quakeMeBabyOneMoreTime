...
export default function MoonEventPage () {
  const [isQuaking, setIsQuaking] = useState(false);
  const waveFormRef = useRef<HTMLDivElement>(null!);
  const waveRef = useRef<WaveSurfer>(null!);
  const [count, setCount] = useState(0)
  const [eventDataInfo, setEventDataInfo] = useState({})

  const url = "/audio/saida_audio.wav";
  const quakeTime = 7;

  useEffect(() => {
    setEventDataInfo(JSON.parse(sessionStorage.getItem("eventData")));
  }, []);
  ...
  return (
    <>
      <div className="hidden lg:flex align-center absolute z-10 left-20 lg:left-40 h-screen">
        <CelestialInfoCard title="Event Data" info={eventDataInfo} />
      </div>
      <div className="hidden lg:flex align-center absolute z-10 right-20 lg:right-40 h-screen">
        <CelestialModelInfoCard title="Event Data" info={eventDataInfo} />
      </div>
          ...
          <DialogContent className="pt-1 text-sm text-white overflow-auto">
            <DialogHeader>
              ...
              <DialogDescription className="text-white max-w-2xl m-auto">
                ...
                <div className="mt-8">
                  <ol>
                      <li><b>filename:</b> {eventDataInfo.filename}</li>
                      <li><b>location:</b> {eventDataInfo.location}</li>
                      <li><b>starttime:</b> {eventDataInfo.starttime}</li>
                      <li><b>endtime:</b> {eventDataInfo.endtime}</li>
                      <li><b>sampling_rate:</b> {eventDataInfo.sampling_rate}</li>
                      <li><b>endtime:</b> {eventDataInfo.endtime}</li>
                      <li><b>sampling_rate:</b> {eventDataInfo.sampling_rate}</li>
                  </ol>
                </div>
